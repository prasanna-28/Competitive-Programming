...
def sol():
        ...
        for idx, i in enumerate(x):
            ...
            if ~trie_A[node + i]:
                ...
                if ~trie_A[node + (1 ^ i)]:
                    depth = max(depth, idx)
                new = [-1, -1, 1]
                trie_A[node + i] = len(trie_A)//3
                trie_A.extend(new)
                node = 3 * (len(trie_A)//3 - 1)
    ...
    for x in b:
        ...
        for idx, i in enumerate(x):
            ...
            if idx in diffs:
                if ~trie_X[node + i]:
                    res += trie_X[3 *trie_X[node+i] + 2]
                if ~trie_X[node + (i ^ 1)]:
                    node = trie_X[node + (i^1)] * 3
                    if idx == B_MAX - 1:
                        res += trie_X[node + 2]
                else:
                    break
