...
def sol():
    ...
    def check(pref, suff):
        ...
        for i in range(n - 1, -1, -1):
            prefmx.append(max(prefmx[-1], pref[i]))
            suffmx.append(max(suffmx[-1], suff[i]))
        prefmx = prefmx[1:]
        suffmx = suffmx[1:]
        prefmx.reverse()
        suffmx.reverse()
        for i in range(n - 1):
            if pref[i] >= 0:
                if prefmx[i + 1] >= pref[i]:
                    return True
                if suffmx[i + 1] >= 0:
                    return True
        return False

    if check(pref, suff) or check(suff[::-1], pref[::-1]):
        print(YES)
    else:
        print(NO)
