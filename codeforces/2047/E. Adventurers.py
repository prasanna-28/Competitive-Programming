...
def sol():
    n = ipt()
    points = [tuple(ivars()) for _ in range(n)]
    points.sort()
    ys = defaultdict(list)
    xs = set()
    for x, y in points:
    ...
    xs = sorted(list(xs))
    left = SortedList()
    right = SortedList([y for _, y in points])
    l = -1 * pow(10, 9)
    l -= 5
    r = pow(10, 9) + 5
    best = 0
    ...
    for x in xs:
        for y in ys[x]:
            right.pop(right.lower_bound(y))
            left.insert(y)
        lft = l
        rgt = r
        while lft <= rgt:
            mid = (lft + rgt)//2
            lftcts1 = left.lower_bound(mid + 1)
            lftcts2 = len(left) - lftcts1
            rgtcts1 = right.lower_bound(mid + 1)
            rgtcts2 = len(right) - rgtcts1
            if (u:=min(lftcts1, lftcts2, rgtcts1, rgtcts2)) >= best:
                best = u
                bestcoord = (x + 1, mid + 1)
            if rgtcts1 + lftcts1 > lftcts2 + rgtcts2:
                rgt = mid - 1
            else:
                lft = mid + 1
